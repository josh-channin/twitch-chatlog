...
function isChatComment(comment) {
    ...
    if (comment.commenter && comment.commenter.display_name) {
        comment.commenter.display_name = hashString(comment.commenter.display_name)
    }

    if (comment.commenter && comment.commenter.name) {
        comment.commenter.name = hashString(comment.commenter.name)
    }
...
function hashString(string) {
    return crypto.createHash('sha256').update(string + 'uOZYwS16Jf').digest('hex');
}
    ...
    _read() {
        ...
        if (this.cursor && offset < this.duration) {
            getChatFragment(this.cursor, this.vodID, this.clientId, this.testServer)
                ...
                .catch((error) => {
                    if (("response" in error && error.response.status === 504) || ("response" in error && error.response.status === 502)) {
                        getChatFragment(this.cursor, this.vodID, this.clientId, this.testServer)
                        .then((response) => {
                            this.cursor = response._next;
                            if (response.comments.length) {
